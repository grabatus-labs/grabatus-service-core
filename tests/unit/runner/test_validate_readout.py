...
from grabatus_service_core.errors import InvalidReadoutError, MissingReadoutError
...
def _valid_payload() -> bytes:
    return make_model_readout().model_dump_json().encode("utf-8")
...
def test_returns_the_parsed_readout_not_merely_the_absence_of_an_error() -> None:
    parsed = validate_readout(result=_result(**{READOUT_OUTPUT_ROLE: _valid_payload()}))
...
def test_other_roles_do_not_satisfy_the_readout_requirement() -> None:
    with pytest.raises(MissingReadoutError) as excinfo:
        validate_readout(result=_result(rules_json=b"{}", summary_csv=b""))
...
def test_a_readout_without_its_explanation_guide_is_rejected() -> None:
    ...
    with pytest.raises(InvalidReadoutError, match="explanation_guide"):
        validate_readout(result=_result(**{READOUT_OUTPUT_ROLE: payload}))


def test_a_readout_describing_no_artifact_is_rejected() -> None:
    payload = _payload_with(artifacts=[])
...
def test_a_run_that_found_nothing_is_still_a_valid_readout() -> None:
    ...
    payload = _payload_with(findings=[], explanation_guide=None)
    body = json.loads(payload)
    body["explanation_guide"] = json.loads(_valid_payload())["explanation_guide"]
    body["explanation_guide"]["recommended_narrative_order"] = []

    parsed = validate_readout(
        result=_result(**{READOUT_OUTPUT_ROLE: json.dumps(body).encode("utf-8")}),
    )
...
def test_unknown_top_level_key_is_rejected() -> None:
    payload = _payload_with(invented_section={"anything": 1})
...
def test_readout_version_the_sdk_does_not_know_is_rejected() -> None:
    payload = _payload_with(readout_version="9.9")
...
def test_the_two_readout_errors_are_not_retriable() -> None:
